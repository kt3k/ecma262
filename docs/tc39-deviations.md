...
| `emu-nt a` 通常リンク色                    | `#333` / `#d0d0d0`  | **`#434343`** / **`#aeaeae`**    | 地の文との比を tc39 より少し高め ≈1.8:1（§2） |
...
> 見出しスケール・`emu-xref` のリンク色・`emu-nt` の hover 青などは tc39 に
> **揃えている**。ここに挙げたのは意図的 に**外した**もの。なお `emu-nt`
> の**通常時のリンク色**は tc39 と絶対値が 異なる。これは §2
> のとおり、地の文とのコントラスト比を tc39（約 1.5:1）より
> **少しだけ高め（≈1.8:1）**に取り、文法シンボルをもう少し浮かせるための調整。

## 1. 本文 line-height（`1.65` / tc39 `1.5`）

- tc39.es: `body { line-height: 1.5 }`
- lume: `main[data-pagefind-body] { line-height: 1.65 }`
...
| モード | tc39.es (`--foreground-color`) | lume (`--fg`) |
| ------ | ------------------------------ | ------------- |
| light  | `#111`                         | `#111827`     |
| dark   | `#fcfcfc`                      | `#e5e7eb`     |

- tc39.es は chromeless なので地の文をほぼ純黒／純白に振っている。
- lume はヘッダ／サイドバー／フッタ（chrome）と同じ `--fg` を本文にも
  使い、ページ全体で配色を統一する方針。tc39 とは light で青み寄りの暗
  グレー、dark で純白より落とした明度になる。

### `emu-nt` リンク色は「地の文との浮き具合」を独自に調整
...
加えて **tc39 の約 1.5:1 自体が lume の地の文に対しては少し平坦に感じる**
ため、文法シンボルをもう少しだけ浮かせる方針で比を **約 1.8:1** に
引き上げた（tc39 より意図的に少し高め）:

| 現行（やや強調） | 地の文    | emu-nt        | 比           | tc39 の比 |
| ---------------- | --------- | ------------- | ------------ | --------- |
| lume light       | `#111827` | **`#434343`** | **1.79 : 1** | 1.49 : 1  |
| lume dark        | `#e5e7eb` | **`#aeaeae`** | **1.79 : 1** | 1.50 : 1  |
...
- lume: `var(--font-sans)`（`-apple-system` 等の system sans）。
...
- tc39.es: グローバル `.secnum` を monospace（`font-feature-settings:"tnum"`）
  でレンダリング。
- lume: 節番号を本文フォント（sans / serif）のまま表示し、monospace に
