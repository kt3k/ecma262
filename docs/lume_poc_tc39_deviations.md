...
| `emu-nt a` 通常リンク色                    | `#333` / `#d0d0d0`  | **`#373737`** / **`#bebebe`**    | 地の文との比を tc39 に合わせる調整（§2） |
...
> **揃えている**（`lume_poc_tc39_alignment.md` 参照）。ここに挙げたのは意図的
> に**外した**もの。なお `emu-nt` の**通常時のリンク色**は tc39 と絶対値が
> 異なるが、これは §2 のとおり「地の文とのコントラスト比」を tc39 に合わせる
> ための調整であり、色を外したのではなく**関係を揃えた**もの。
...
### `emu-nt` リンク色は「コントラスト比」を tc39 に合わせて調整

地の文色を独自に維持すると、`emu-nt a` の通常リンク色に tc39 と同じ
グレー（`#333` / `#d0d0d0`）をそのまま使った場合、「地の文 ↔ emu-nt」の
コントラスト比がずれてしまう（特に dark で平坦になる）:
...
そこで **emu-nt の通常リンク色を、lume の地の文に対して tc39 と同じ比に
なるグレーへ調整した**。色の絶対値は tc39 と少し変わるが、見た目上の
「地の文からの浮き具合」は tc39 と一致する:

| 現行（調整後） | 地の文    | emu-nt        | 比       | tc39 の比 |
| -------------- | --------- | ------------- | -------- | --------- |
| lume light     | `#111827` | **`#373737`** | 1.49 : 1 | 1.49 : 1  |
| lume dark      | `#e5e7eb` | **`#bebebe`** | 1.50 : 1 | 1.50 : 1  |
...
該当: 本文色は `lume-poc/styles.css` の `:root` / `html.dark` の `--fg`、 emu-nt
