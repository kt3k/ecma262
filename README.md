...
- **draft … ES2015** are ecmarkup _source_; `lume/scripts/build-chapters.mjs`
...
- **ES5.1** predates ecmarkup, so its already-rendered official HTML goes
  through `lume/scripts/build-chapters-es51.mjs`, a re-skin ingester (see
...
2. Append `{ "id": "<id>", "title": "…" }` to `lume/editions.json`
   (newest-first).
